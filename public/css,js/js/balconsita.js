    ...
    btnAceitar.forEach(button => {
        button.addEventListener('click', function() {
            ...
            novoPedido.innerHTML = `
                ...
                <p class="p-preco-total">${precoTotal}</p>
            ...
            novoPedido.querySelector('.btn-mover1').addEventListener('click', function() {
                ...
                pedidoCompleto.innerHTML = `
                    ...
                    <p class="p-mesas">${mesa}</p>
                    <p class="p-qtd">${quantidade}</p>
                    <p class="p-preco-total1">${precoTotal}</p>
                `;
            ...
            // Adiciona o evento de clique ao botão "Remover" de maneira dinâmica
            novoPedido.addEventListener('click', function(event) {
                if (event.target && event.target.matches('.bnt-remover')) {
                    // Remove o pedido do container de pedidos aceitos
                    novoPedido.remove();
                }
            });
            ...
            // Remove o pedido original do container de pedidos em processo
            pedidoDiv.remove();
        });
    });
});
